...
def download_from_modelscope(model_id, origin_file_path, local_dir):
    ...
    if file_name in os.listdir(local_dir):
        ...
        try:
            ...
            if error.name != "modelscope":
                raise
            raise ModuleNotFoundError(
                "ModelScope downloads require the optional 'modelscope' package; "
                "install it only when download_resource='ModelScope'. Local model "
                "paths and Hugging Face downloads do not require ModelScope."
            ) from error

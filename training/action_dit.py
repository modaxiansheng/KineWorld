    ...
    def forward(
        ...
        for i, block in enumerate(self.blocks):
            ...
            if use_gradient_checkpointing and self.training:
                # Bind the current block as a default argument.  A plain loop
                # closure would otherwise point every backward recomputation
                # at the final block after the loop has completed.
                def checkpointed_block(
                    x_, vfeat_, t_mod_, text_ctx_, rope_, block_=block
                ):
                    return block_(
                        x_, vfeat_, t_mod_, text_ctx_, rope=rope_
                    )
